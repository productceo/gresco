"""A Wrapper around DecoderRNN that can be used to do beam searches.
"""
...
def _inflate(tensor, times, dim):
        ...
        if tensor_dim is 3:
            b = tensor.size(dim)
            if dim == 0:
                return tensor.repeat(1, 1, times).view(b * times, tensor.size(1), -1)
            if dim == 1:
...
class TopKDecoder(torch.nn.Module):
    ...
    Inputs:
        - **inputs** (seq_len, batch, input_size): List of sequences, whose
            length is the batch size and within which each sequence is a
            list of token IDs. It is used for teacher forcing when provided.
            (default is `None`)
        - **encoder_hidden** (batch, seq_len, hidden_size): Tensor containing
            the features in the hidden state `h` of encoder. Used as the
            initial hidden state of the decoder.
        - **encoder_outputs** (batch, seq_len, hidden_size): Tensor with
            containing the outputs of the encoder. Used for attention
            mechanism (default is `None`).
        - **function** (torch.nn.Module): A function used to generate
            symbols from RNN hidden state
            (default is `torch.nn.functional.log_softmax`).
        - **teacher_forcing_ratio** (float): The probability that teacher
            forcing will be used. A random number is drawn uniformly
            from 0-1 for every decoding token, and if the sample is
            smaller than the given value, teacher forcing would be used
            (default is 0).

    Outputs:
        - **decoder_outputs** (batch): Batch-length list of tensors with size
            (max_length, hidden_size) containing the outputs of the decoder.
        - **decoder_hidden** (num_layers * num_directions, batch, hidden_size):
            Tensor containing the last hidden state of the decoder.
        - **ret_dict**: dictionary containing the following:
            - *length*: List of integers representing lengths of output
                sequences.
            - *topk_length*: List of integers representing lengths of
                beam search sequences.
            - *sequence*: List of sequences, where each sequence is a
                list of predicted token IDs
            - *topk_sequence*: list of beam search sequences, each beam
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None,
        ...
        """Forward rnn for MAX_LENGTH steps. Look at
        :func:`seq2seq.models.DecoderRNN.DecoderRNN.forward_rnn`
        for details.
        """
        ...
        sequence_scores.index_fill_(0, torch.LongTensor(
            [i * self.k for i in range(0, batch_size)]), 0.0)
        sequence_scores = Variable(sequence_scores)
        # Initialize the input vector
        input_var = Variable(torch.transpose(torch.LongTensor(
            [[self.SOS] * batch_size * self.k]), 0, 1))
        ...
        for _ in range(0, max_length):
            ...
            # If doing local backprop (e.g. supervised training),
            # retain the output layer
            if retain_output_probs:
                stored_outputs.append(log_softmax_output)

            # To get the full sequence scores for the new candidates,
            # add the local scores for t_i to the predecessor scores for
            # t_(i-1).
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors,
        ...
        for b_idx in range(b):
            l[b_idx] = [l[b_idx][k_idx.data.item()] for k_idx in re_sorted_idx[b_idx, :]]
