...
class EncoderRNN(BaseRNN):
    ...
    def __init__(self, vocab_size, max_len, hidden_size,
                 ...
                 vocab_embed_size=None, embedding=None):
        super(EncoderRNN, self).__init__(vocab_size, max_len, hidden_size,
                input_dropout_p, dropout_p, n_layers, rnn_cell)
        self.variable_lengths = variable_lengths
        if vocab_embed_size is None:
            vocab_embed_size = hidden_size
        self.embedding = nn.Embedding(vocab_size, vocab_embed_size)
        self.pretrained_embedding = embedding is not None
        if self.pretrained_embedding:
            self.embedding.weight = nn.Parameter(embedding, requires_grad=False)
        self.rnn = self.rnn_cell(vocab_embed_size, hidden_size, n_layers,
